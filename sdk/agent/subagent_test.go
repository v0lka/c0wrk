...
func TestRunSubAgent_MaxStepsExhausted(t *testing.T) {
	...
	cm := newMockContextManager()
	exec := NewExecutor(mockLLM, newMockToolExecutor(), &mockTokenCounter{}, 2, nil, nil, false, ToolResultBudget{})
...
func TestRunSubAgentsParallel_MultipleAgents(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		agents[i] = SubAgentTask{
			...
			Executor: NewExecutor(mockLLM, newMockToolExecutor(), &mockTokenCounter{}, 10, nil, nil, false, ToolResultBudget{}),
			CM:       newMockContextManager(),
...
func TestRunSubAgentsParallel_MixedResults(t *testing.T) {
	...
	agents := []SubAgentTask{
		{
			StepID:   "step_1",
			Executor: NewExecutor(successLLM, newMockToolExecutor(), &mockTokenCounter{}, 10, nil, nil, false, ToolResultBudget{}),
			CM:       newMockContextManager(),
			TaskDesc: "good task",
		},
		{
			StepID:   "step_2",
			Executor: NewExecutor(failLLM, newMockToolExecutor(), &mockTokenCounter{}, 10, nil, nil, false, ToolResultBudget{}),
			CM:       newMockContextManager(),
			TaskDesc: "bad task",
		},
	}
