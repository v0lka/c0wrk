...
- **Every automatic decision is auditable.** Each resolved gate emits a non-blocking, persisted `autonomy_decision` session event (`kind` = `tool_confirm`|`assisted_deny`|`step_limit`, `mode` = the autonomy posture `silent`|`assisted`, `policy` = the sub-policy, `verdict`, `tool`/`reason`, `justification`), rendered as a status notice and reconstructed identically on reload — the receipt chain ASI10 requires. (The other two sub-policies need no separate event: `ask_user: disable` reaches the model as the tool's explicit "not available" result, and `review_prompt: suppress` emits no prompt.)
