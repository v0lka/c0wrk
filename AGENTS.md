...
- **Hard external runtime deps** verified at startup by `desktop.verifyExternalDependencies` (fatal modal + quit on absence): `git` (used by workspace + vector index branch detection). All other tools — `rg` (ripgrep), `rtk`, `uv`, `markitdown` — are managed by the tool-manager (`core/toolmanager/`) and auto-downloaded on first run to `~/.c0wrk/tools/`. See `specs/decisions/010-tool-manager.md`.
...
- **Linters enabled** (see `.golangci.yml`): `errcheck` (incl. type assertions), `govet`, `staticcheck`, `ineffassign`, `unused`, `errorlint`, `nilerr`, `gocritic` (diagnostic+performance+style, except `hugeParam`/`rangeValCopy`), `revive` with `exported` & `var-naming` disabled, `prealloc`, `bodyclose`, `noctx`, `sqlclosecheck`, `perfsprint`, `unconvert`, `wastedassign`, `copyloopvar`, `durationcheck`, `whitespace`. Run `make lint` before declaring done.
- **Tool registry pattern**: built-in tools live in `sdk/tools/builtins/`; MCP-backed tools are added at runtime via `core/tools/mcp/gateway.go`. To add a new built-in tool, register it in `sdk/tools/builtins` and wire it through `tools.RegisterBuiltinTools` (defined in `core/tools/builtin_registration.go`, called from `core/builder.go`).
