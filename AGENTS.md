...
- **Tool registry pattern**: reusable built-in tools live in `sdk/tools/builtins/`; c0wrk-specific tools (e.g. `ask_user`) live in `core/tools/`. MCP-backed tools are added at runtime via `core/tools/mcp/gateway.go`. To add a new built-in tool, implement `tools.Tool` in the correct package and wire it through `core/tools.RegisterBuiltinTools` (defined in `core/tools/builtin_registration.go`, called from `core/builder.go`).
