...
- **Desktop API surface**: methods on `*desktop.App` exposed to the frontend are split across `desktop/api_*.go` files by area (`config`, `mcp`, `project`, `session`, `workspace`). New frontend-callable methods go in the matching `api_*.go`.
