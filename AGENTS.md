...
- **Hard external runtime deps** verified at startup by `desktop.verifyExternalDependencies` (fatal modal + quit on absence): `git` (used by workspace + vector index branch detection) and `rg` (ripgrep, used by the `ripgrep` built-in tool). See `specs/decisions/004-external-binary-dependencies.md`.
