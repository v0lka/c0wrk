...
- "structural": The task requires capabilities beyond simple tool chaining — multi-file coordination, complex dependency management, iterative refinement with state. Suggest "replan".
- "wrong_approach": The executor took a fundamentally wrong path (e.g., searched the wrong sources, misunderstood the task, used inappropriate tools). Suggest "replan" — retrying the same framing rarely succeeds.
...
When in doubt between "retry" and "replan", prefer "replan" — experiments show that retrying a fundamentally wrong approach wastes execution budget without improving outcomes.
