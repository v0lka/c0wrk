...
func testThresholds() config.CompactionThresholds {
	return config.CompactionThresholds{
...
func makeStep(thought, observation string, toolID int) core.Step {
	return core.Step{
		Thought: thought,
		Action: llm.ToolCall{
...
func TestBuildPromptOrdering(t *testing.T) {
	...
	criteria := []core.AcceptanceCriterion{
		...
		{ID: "ac_2", Description: "Second criterion", CheckType: "programmatic", CheckCmd: "go test"},
	}
	cw.SetTask("Complete the coding task", criteria)
