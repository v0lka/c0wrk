...
Инфраструктура сборки имеет платформенно-зависимые определения в Makefile (`APP_BUNDLE_DIR` и `APP_MODELS_DIR`: Darwin → `.app/Contents/…`, Linux → `build/bin`, else → `build/bin`), но сам `make` недоступен на Windows без MSYS2. `resolveModelPath()` в `desktop/startup.go` поддерживает flat-структуру `exeDir/models/` (работает на Windows и Linux).
...
| PTY-терминал          | Заглушка (ошибка)  | `core/terminal/manager_stub.go` возвращает «not supported»                                                      |
| `bash_exec` tool      | **Отсутствует**    | Нет Windows-заглушки; shell-альтернатива не реализована                                                        |
| ONNX Runtime          | Частично           | `APP_BUNDLE_DIR` платформенно-зависим (Windows → `build/bin`), но `fetch-onnx` требует POSIX shell             |
...
func (t *BashExecTool) ParameterSchema() json.RawMessage { return nil }
...
**Рекомендация для MVP**: PowerShell fallback. PowerShell предустановлен на всех Windows 10+, не требует дополнительных зависимостей.

**Файл**: `sdk/tools/builtins/bash_windows.go`

**Реализация**: реализовать `Execute()` через `powershell -Command "..."`:
...
func (t *BashExecTool) Execute(ctx context.Context, raw json.RawMessage, tc agent.ToolContext) (string, error) {
    var params struct {
...
**Перспектива (не в MVP)**: автоопределение Git Bash (`C:\Program Files\Git\bin\bash.exe`) или WSL (`wsl.exe`) как приоритетной shell-среды, если они установлены. Потребует абстракции `ShellProvider`:
...
**Реализация**: добавить Windows job:
...
| 3.2   | `sdk/tools/builtins/bash_windows.go` | **Новый** — PowerShell shell-альтернатива                    | 3    | Отсутствует |
...
**Общий объём оставшихся работ**: ~80 строк Go (заглушки), ~60 строк Go (PowerShell shell), ~80 строк PowerShell, ~10 строк Go (терминал), ~30 строк README, ~30 строк CI YAML, ~5 строк .gitignore.
