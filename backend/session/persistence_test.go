...
// ---------------------------------------------------------------------------
// PlanReviewStore tests
// ---------------------------------------------------------------------------

func TestUpdateSessionPlanReview(t *testing.T) {
	store, cleanup := setupTestStore(t)
	defer cleanup()
	...
	// Set plan review state
	if err := store.UpdateSessionPlanReview(context.Background(), session.ID, "awaiting_accept", "/tmp/test/plan.md"); err != nil {
		t.Fatalf("UpdateSessionPlanReview failed: %v", err)
	}
	...
	if loaded.PlanReviewPhase != "awaiting_accept" {
		t.Errorf("expected phase 'awaiting_accept', got %q", loaded.PlanReviewPhase)
	}
	if loaded.PlanReviewPath != "/tmp/test/plan.md" {
		t.Errorf("expected path '/tmp/test/plan.md', got %q", loaded.PlanReviewPath)
	}

	// Clear plan review state
	if err := store.UpdateSessionPlanReview(context.Background(), session.ID, "", ""); err != nil {
		t.Fatalf("UpdateSessionPlanReview clear failed: %v", err)
	}
	...
	if loaded.PlanReviewPhase != "" {
		t.Errorf("expected empty phase after clear, got %q", loaded.PlanReviewPhase)
	}
	if loaded.PlanReviewPath != "" {
		t.Errorf("expected empty path after clear, got %q", loaded.PlanReviewPath)
	}
	// plan_review_context should also be cleared
	if loaded.PlanReviewContext != "" {
		t.Errorf("expected empty context after clear, got %q", loaded.PlanReviewContext)
	}
}

func TestUpdateSessionPlanReviewContext(t *testing.T) {
	store, cleanup := setupTestStore(t)
	defer cleanup()

	session := SessionInfo{
		ID:        "plan-review-ctx-test",
		ProjectID: testProjectID,
		Name:      "Plan Review Context Test",
		CreatedAt: time.Now().Format(time.RFC3339),
	}
	if err := store.SaveSession(context.Background(), session); err != nil {
		t.Fatalf("failed to save session: %v", err)
	...
	contextJSON := `{"msg":"build a CLI","mode":"advanced","skills":["go"]}`
	if err := store.UpdateSessionPlanReviewContext(context.Background(), session.ID, "awaiting_feedback", "/tmp/prev-plan.md", contextJSON); err != nil {
		t.Fatalf("UpdateSessionPlanReviewContext failed: %v", err)
	}
	...
	if loaded.PlanReviewPhase != "awaiting_feedback" {
		t.Errorf("expected phase 'awaiting_feedback', got %q", loaded.PlanReviewPhase)
	}
	if loaded.PlanReviewPath != "/tmp/prev-plan.md" {
		t.Errorf("expected path '/tmp/prev-plan.md', got %q", loaded.PlanReviewPath)
	}
	if loaded.PlanReviewContext != contextJSON {
		t.Errorf("expected context %q, got %q", contextJSON, loaded.PlanReviewContext)
	}
}

func TestGetSessionsInPlanReview(t *testing.T) {
	store, cleanup := setupTestStore(t)
	defer cleanup()

	// Create sessions in different review phases
	s1 := SessionInfo{
		ID:              "pr-session-1",
		ProjectID:       testProjectID,
		Name:            "PR Session 1",
	...
	s2 := SessionInfo{
		...
		Name:            "PR Session 2",
		CreatedAt:       "2024-01-15T11:00:00Z",
		PlanReviewPhase: "awaiting_feedback",
		PlanReviewPath:  "/tmp/plan2.md",
	}
	s3 := SessionInfo{
		ID:        "pr-session-3",
		ProjectID: testProjectID,
		Name:      "Normal Session",
		CreatedAt: "2024-01-15T12:00:00Z",
		// No plan review phase — should NOT appear in results
	}

	for _, s := range []SessionInfo{s1, s2, s3} {
		if err := store.SaveSession(context.Background(), s); err != nil {
			t.Fatalf("failed to save session %s: %v", s.ID, err)
		}
	}

	// Query sessions in plan review
	inReview, err := store.GetSessionsInPlanReview(context.Background(), testProjectID)
	if err != nil {
		t.Fatalf("GetSessionsInPlanReview failed: %v", err)
	}
	if len(inReview) != 2 {
		t.Fatalf("expected 2 sessions in plan review, got %d", len(inReview))
	}

	// Verify ordering (by last_active_at DESC — s2 created later)
	if inReview[0].ID != "pr-session-2" {
		t.Errorf("expected newest first (pr-session-2), got %q", inReview[0].ID)
	}
	if inReview[1].ID != "pr-session-1" {
		t.Errorf("expected second (pr-session-1), got %q", inReview[1].ID)
	}

	// Verify plan review fields are populated
	if inReview[0].PlanReviewPhase != "awaiting_feedback" {
		t.Errorf("expected phase 'awaiting_feedback', got %q", inReview[0].PlanReviewPhase)
	}
	if inReview[1].PlanReviewPath != "/tmp/plan1.md" {
		t.Errorf("expected path '/tmp/plan1.md', got %q", inReview[1].PlanReviewPath)
	}

	// Query for a different project — should return empty
	empty, err := store.GetSessionsInPlanReview(context.Background(), "other-project")
	if err != nil {
		t.Fatalf("GetSessionsInPlanReview for other project failed: %v", err)
	}
	if len(empty) != 0 {
		t.Errorf("expected empty for other project, got %d", len(empty))
	}
	if empty == nil {
