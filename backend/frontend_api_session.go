...
func (f *FrontendAPI) SendMessage(id, text, mode string, activeSkills []string, modelOverride, reasoningEffort string) error {
	if f.app == nil || f.app.Manager() == nil {
	...
	if err := f.app.Manager().SendMessage(f.ctx(), id, processedText, mode, activeSkills, modelOverride, reasoningEffort); err != nil {
