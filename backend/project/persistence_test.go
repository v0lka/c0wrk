...
func TestCreateTablesRetainsLegacyResearchRootColumn(t *testing.T) {
	...
	// Simulate a database written by an older build that still had the
	// research_root column.
...
func TestCreateTablesMigrationResearchPins_LegacyTableUpgraded(t *testing.T) {
	...
	if loaded.Name != "Legacy" || loaded.WorkspacePath != "/tmp/legacy" {
		t.Errorf("legacy project fields not preserved: %+v", loaded)
	}
	if !reflect.DeepEqual(loaded.ResearchPins, ResearchPins{}) {
