...
func (a *App) InstallRtk() error {
	progress := func(status string) {
		wailsRuntime.EventsEmit(a.ctx, "rtk:install-progress", status)
	}

	installPath, err := beRtk.InstallRtk(progress)
	if err != nil {
		return err
	}
	...
	wailsRuntime.EventsEmit(a.ctx, "rtk:status", status)

	wailsRuntime.EventsEmit(a.ctx, "rtk:install-progress", "done")
	return nil
}
