...
### Added
- **Vector-index memory program: GC soft limit, content-less index, park budget, live RSS indicator** — the desktop process's multi-gigabyte indexing footprint is attacked on four fronts. A Go runtime soft memory limit (`runtime.memory_soft_limit_mb`, default AUTO = clamp(50% physical RAM, 2–8 GiB); `>0` = explicit MiB; `-1` = off; an explicit value beats a `GOMEMLIMIT` env var, which beats AUTO) plus `FreeOSMemory` after every full indexing pass, park eviction, and migration remove the ×2 GC overlay and hand RAM back to the OS. The vector index becomes content-less — documents store ID + embedding + the four chunk-position metadata fields, per-file bookkeeping moves to the file-hash sidecar, and chunk text is reconstructed best-effort from source files at search time (a missing file degrades to an explicit placeholder, never an error, and `must_match` excludes it) — with a one-time background migration per collection (marker-gated, cancel-safe, replays on the next open; shrunken per-document gobs are rewritten in place). The sidecar entry grows a 5th field — the committed chunk-index set — so full-collection enumeration, Browse, and lexical rebuilds no longer materialize every document or spend ONNX inference on enumeration. Project parking is bounded by bytes as well as count (`vector_index.park_budget_mb`, default 1024 MiB: the oldest parked states are evicted — sidecar flushed, RAM freed — while the summed estimated footprints exceed it; the effective bound is `min(park_capacity, park_budget_mb)`). A live `RSS NNNN MB` status-bar indicator (5 s polling, exact-bytes tooltip, silent when unavailable) makes the program's effect observable in-app. See [ADR-049](./specs/decisions/049-vector-index-memory-reduction.md).
