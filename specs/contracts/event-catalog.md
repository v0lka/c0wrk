...
| `compaction_finished`| `{strategy, success, cancelled?, error?, before_percent, after_percent, resumed?, paused_without_resume?}`                     | useContextEvents | Manual compaction terminal state (exactly one of success/cancelled/error). `resumed` is true when the flow auto-resumed the task it paused; `paused_without_resume` is true when that auto-resume FAILED — a paused checkpoint remains that the UI never saw (session_paused is suppressed while compacting), so the client re-applies the paused state. Transient (not persisted) |
