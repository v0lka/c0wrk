...
| `workspace:tree_changed` | backend → frontend | `{research_scoped: boolean}`                                                                                                                                                                                                   | backend/frontend_api_project.go | File tree modified (workspace watcher callback)             |
...
| `agent_metrics`     | `{finish, parse_errors, nudges{repeat,same_tool,fruitless,parse}, aborts{...}, steps, output_tokens, small_llm{enabled, variants[]}}` | useLifecycleEvents | Aggregated per-run agent quality report emitted once on task finish/abort (`finish`: `full`\|`partial`\|`failed`\|`aborted`\|`cancelled`). Counters are accumulated by the session-layer aggregator from `executor_diagnostic` events (loop-detector nudges/aborts, parse-error nudges), `step_start`, and session token totals. Collected and persisted for every run regardless of the small-LLM profile or UI settings (role `status`). Rendered nowhere in the chat: live and reload both write the latest report to `planStore.sessionStats.lastAgentMetrics` (ExecutionPanels stats row); the stats row itself is opt-in — the user enables it in Settings → General ("Session Statistics", off by default), which gates display only, never collection; on reload the row restores that store and is filtered out of the message list (see `lastAgentMetricsFromHistory`). |
