...
| `vector_index:status`    | backend → frontend | `{state, progress, files_indexed, total_files, current_file?, branch?, phase?, indices?}` (see `VectorIndexStatus`; `phase` is one of `both` \| `embedding` \| `lexical`; `indices` lists the indices being written) | backend/frontend_api_project.go | Vector index status update                                  |
...
| `tool_manager:progress` | backend → frontend | `{tool: string, stage: string, bytes_done: int64, bytes_total: int64}` (stage is `download` \| `extract` \| `python_bootstrap`; `bytes_total=0` means indeterminate)                     | core/toolmanager/manager.go    | Per-tool progress update (bytes reported at ~100ms intervals)  |
