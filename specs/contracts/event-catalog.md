...
| `compaction_finished`| `{strategy, success, cancelled?, error?, before_percent, after_percent, resumed?, paused_without_resume?, nothing_compacted?, deferred_to_resume?}` | useContextEvents | Manual compaction terminal state (exactly one of success/cancelled/error; a no-op counts as success). `resumed` is true when the flow auto-resumed the task it paused; `paused_without_resume` is true when that auto-resume FAILED — a paused checkpoint remains that the UI never saw (session_paused is suppressed while compacting), so the client re-applies the paused state. `nothing_compacted` is true when the dialogue already fit within the strategy's limits (zero percents, no marker row, and no `context_compaction` card follows); `deferred_to_resume` is true when such a no-op armed the one-shot resume compaction for a waiting paused task — the resumed run force-compacts the merged trajectory and the real numbers arrive as the executor's `context_compaction` card. Transient (not persisted) |
