...
| `project:git_config_risk` | backend → frontend | `{path: string, source: "project" \| "workdir", notice: string, findings: [{key: string, description: string}]}` (`findings` is never empty; `key` is a full config key like `core.fsmonitor` or a synthetic marker like `(config unreadable)`) | backend/frontend_api_gitconfig_risk.go (called from `SwitchProject` in frontend_api_project.go and `AddWorkDirectory` in frontend_api_workdirs.go) | Dangerous keys detected in the `.git/config` of a newly opened project or added work directory (exec-free scan via `core/workspace.ScanGitConfig`): command-bearing filters, merge drivers, textconv, `core.fsmonitor`, `core.hooksPath`, `attr.tree`, include directives, malformed/unreadable config — detection only, actual neutralization happens in the git spawn layer. `notice` states explicitly that repository-defined hooks do not run inside c0wrk. A clean, fully visible config emits nothing, and so does a repository root the user marked trusted (`security.trusted_git_repos`, maintained via the `TrustGitRepo`/`RemoveTrustedGitRepo`/`GetTrustedGitRepos` RPCs — exact-path match; suppression is warning-only, neutralization stays in force) |
...
| `research:changed`       | backend → frontend | `{project_id: string, action: "enabled" \| "disabled"}`                                                                                                                                                              | backend/frontend_api_research.go | RESEARCH mode toggled for a project (`EnableResearch` / `DisableResearch`); the Research panel and project list refresh |
...
| `compaction_finished`| `{strategy, success, cancelled?, error?, before_percent, after_percent, resumed?, paused_without_resume?, nothing_compacted?, deferred_to_resume?, compaction_noop?}` | useContextEvents | Manual compaction terminal state (exactly one of success/cancelled/error; a no-op counts as success). `resumed` is true when the flow auto-resumed the task it paused; `paused_without_resume` is true when the task was left paused without the flow's auto-resume — either the auto-resume failed, or the flow honoured a user-owned pause (a user-initiated pause is never stolen by the compaction; see [../domains/memory/compaction.md](../domains/memory/compaction.md) § Manual Context Compaction) — a paused checkpoint remains that the UI never saw (session_paused is suppressed while compacting), so the client re-applies the paused state. `nothing_compacted` is true when the dialogue already fit the manual-compaction budget (target % of the effective window; count-based limits on an unknown window) — zero percents, no marker row, and no `context_compaction` card follows; `deferred_to_resume` is true when such a no-op armed the one-shot resume compaction for a waiting paused task — the resumed run force-compacts the merged trajectory and the real numbers arrive as the executor's `context_compaction` card. `compaction_noop` recomputes the would-no-op prediction on the POST-flow history (true after a successful compaction and after the no-op outcome; the untouched history's own verdict after cancel/error) so the UI refreshes the compact button's disabled state without a status refetch — see [../domains/memory/compaction.md](../domains/memory/compaction.md). Transient (not persisted) |
