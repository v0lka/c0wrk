...
| `agent_metrics`     | `{finish, parse_errors, nudges{repeat,same_tool,fruitless,parse}, aborts{...}, steps, output_tokens, small_llm{enabled, variants[]}}` | useLifecycleEvents | Aggregated per-run agent quality report emitted once on task finish/abort (`finish`: `full`\|`partial`\|`failed`\|`aborted`\|`cancelled`). Counters are accumulated by the session-layer aggregator from `executor_diagnostic` events (loop-detector nudges/aborts, parse-error nudges), `step_start`, and session token totals. Collected for every run regardless of the small-LLM profile. Persisted (role `status`) but rendered nowhere in the chat: live and reload both write the latest report to `planStore.sessionStats.lastAgentMetrics` (ExecutionPanels stats row); on reload the row restores that store and is filtered out of the message list (see `lastAgentMetricsFromHistory`). |
