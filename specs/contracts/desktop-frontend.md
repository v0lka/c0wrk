...
| `GetShellExecSettings`   | —                        | ShellExecSettingsResponse         | Get the `shell_exec` launch-shape override (both platform sections; empty command = built-in `bash -c` / powershell `-Command` shape). Pure in-memory read; command slices are deep-copied. See [ADR-057](../decisions/057-shell-execution-override.md) |
