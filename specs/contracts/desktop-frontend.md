...
| `Commit`               | message, force          | (CommitResult, error)         | Create a commit (suppression-aware: an untrusted repo arming commit hooks/signing and not forced is withheld with a non-nil `Suppressed{Hooks, SigningRepo, SigningGlobal}` and a nil error; `force=true` commits hardened; a trusted repo commits raw — [ADR-054](../decisions/054-commit-hooks-signing-gate.md)) |
