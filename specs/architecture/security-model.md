...
**Accepted trade-off:** legitimate hooks (husky, pre-commit) and LFS smudge/clean never run inside c0wrk — filters are distrusted unconditionally; hook stripping is *strip-and-warn*, never silent and never operation-failing.

Source: `internal/sysproc/git.go` (`gitSafetyOverrides`), `core/workspace/gitconfig.go` (`ScanGitConfig`, `NeutralizingArgv`), `core/workspace/git.go` (`GitCmdInRepo`), `backend/frontend_api_gitconfig_risk.go` (`notifyGitConfigRisk`), `sp4rk/tools/builtins/paths.go` (`isPathInGitDir`).
