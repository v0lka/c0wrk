...
**Accepted trade-off:** legitimate hooks (husky, pre-commit) and LFS smudge/clean do not run inside c0wrk by default — filters are distrusted; hook stripping is *strip-and-warn*, never silent and never operation-failing. The one exception is the commit boundary ([ADR-054](../decisions/054-commit-hooks-signing-gate.md)): an untrusted repository arming commit-family hooks or signing has its commit **withheld** (a non-error `Suppressed` result the frontend renders as an explicit force-or-trust decision) instead of landing stripped — a commit is a persistent artifact, so the strip is decided before it is created. A trusted repository opts out of this (its own hooks/filters/signing run), but the opt-out is snapshot-bound and fails closed back to hardening on any config drift.
