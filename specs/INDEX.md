...
- [workspace.md](domains/workspace.md) - File tree, vector index, workspace watcher; vector-index memory program (content-less index + one-time migration, park byte budget, GC soft limit, RSS indicator — [ADR-049](decisions/049-vector-index-memory-reduction.md))
...
- [050-papers-library.md](decisions/050-papers-library.md) - Papers library: the `study-paper` skill is vendored (forked into) the repo as an embedded, independently-versioned pack (`core/papers/skills/`); the library lives at `<research-root>/papers/` (default `<workspace>/.research/papers`) as a global sibling of the `R-NNN` projects, watched independently of RESEARCH, with comparisons at `<research-root>/comparisons/`. The original "hybrid" global seeding decision is superseded in part by ADR-051
- [051-research-pack-reconciliation.md](decisions/051-research-pack-reconciliation.md) - Research pack reconciliation: one path (`reconcileResearchPacks`) seeds every c0wrk-owned pack PROJECT-LOCALLY (research-* skills, `study-paper`, research agent profile) on research enable and on every switch to a research-enabled project — seeding missing entries, upgrading pack-marked outdated ones, preserving user-owned dirs; the global `~/.c0wrk/.agents/skills` startup seed is removed because a same-named `~/.agents` skill outranked it in discovery (first-wins), and only the project-local copy reliably wins; `RunPaperLiterature` resolves `literature.py` from the requesting project's local copy
- [052-per-provider-tls-pinning.md](decisions/052-per-provider-tls-pinning.md) - Per-provider TLS verification override via SPKI pinning: one config key, `tls_fingerprint` (base64 SHA-256 SPKI pin, Chromium/RFC 7469 style) on both compatible provider families — **the pin is the only switch** (non-empty = only the pinned key is accepted, empty = system CA verification; no toggle, no configured accept-any state). sp4rk `ProviderEntry.HTTPClient` per-entry client override keeps TLS policy in c0wrk; `core/llmtls` mechanics + credential-less Get-fingerprint RPC; all four dial paths (router, Fetch Models, lazy probe, draft) wrapped consistently; pointer-sentinel round-trip (`TLSFingerprint *string` at the API boundary only; nil = keep, `""` = clear) keeps debounced partial saves from dropping the pin → The pin-vs-proxy interaction is settled by ADR-053 (proxy wins)
- [053-proxy-wins-over-tls-pin.md](decisions/053-proxy-wins-over-tls-pin.md) - Proxy wins over the per-provider TLS pin (amends ADR-052): with an effective proxy (`enabled` AND a non-empty `url`, the `proxy.BuildTransport` rule) the pin is ignored on every dial path — `llmtls.ResolveProviderClient` returns the plain proxy client verbatim, router entries carry no per-entry client, and the Get-fingerprint RPC rejects with an actionable error; the Settings UI disables the whole TLS section with an explanatory comment while the proxy is active; `proxy.bypass_list` is the documented route for running both at once (bypassed host → direct dial → pin applies). Without a proxy, ADR-052 semantics are unchanged.
