...
- [052-per-provider-tls-pinning.md](decisions/052-per-provider-tls-pinning.md) - Per-provider TLS verification override via SPKI pinning: one config key, `tls_fingerprint` (base64 SHA-256 SPKI pin, Chromium/RFC 7469 style) on both compatible provider families — **the pin is the only switch** (non-empty = only the pinned key is accepted, empty = system CA verification; no toggle, no configured accept-any state, and no key at all on the fixed providers). **Proxy wins**: with an effective proxy (`enabled` AND a non-empty `url`) the pin is ignored on every dial path and the UI disables the whole section, with `proxy.bypass_list` as the documented route for running both at once. Two resolvers, not one (`llmtls.RouterEntryClient` returns nil so the router-level client keeps the long LLM timeout; `llmtls.DirectDialClient` returns the proxy client verbatim where no such fallback exists). sp4rk `ProviderEntry.HTTPClient` keeps TLS policy in c0wrk; `core/llmtls` holds the mechanics; the credential-less Get-fingerprint RPC is unconditional with respect to any configured pin and rejects while a proxy is active. Pointer-sentinel round-trip (`*string` at the API boundary only; nil = keep, `""` = clear) keeps debounced partial saves from dropping the pin; a draft Zustand store (not a config re-read) gates the UI across settings tabs. Also fixes `UpdateProxySettings` holding configMu across `RebuildProxy`
