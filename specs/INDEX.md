...
- [045-cheap-status-dot-animation.md](decisions/045-cheap-status-dot-animation.md) - Long-lived status indicators (chat activity dot, index-build dots) use the stock `animate-pulse` opacity blink instead of the transform-based `animate-ping`: under WebKitGTK software rendering (Wails default) a transform animation re-rasterizes every frame and burned a full CPU core while a session was visible; opacity is compositor-cheap (measured ~0% CPU); GPU-compositing via `WebviewGpuPolicy` was field-tested and rejected (blank window on both test machines, wails#2977); `animate-spin` short-lived loaders stay transform-based; new long-lived indicators must be opacity-only
