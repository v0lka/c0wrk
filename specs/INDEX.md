...
- [050-papers-library.md](decisions/050-papers-library.md) - Papers library: the `study-paper` skill is vendored (forked into) the repo as an embedded, independently-versioned pack (`core/papers/skills/`), seeded "hybrid" — once, globally, into `~/.c0wrk/.agents/skills` at startup, independent of the RESEARCH toggle and of any project; the library lives at `<research-root>/papers/` (default `<workspace>/.research/papers`) as a global sibling of the `R-NNN` projects, watched independently of RESEARCH, with comparisons at `<research-root>/comparisons/`
