...
> **Drift note (2026-08-25, vibespec-check):** mvdan.cc/sh/v3/syntax is confined to sp4rk/tools/symlink.go again — sp4rk/tools/shellEnvBindings.go (and with it `collectCommandEnvBindings`) was deleted by [ADR-060](./055-symlink-gate-literal-paths-only.md), removing the last other importer. The import remains inside the sp4rk `tools` package only.
