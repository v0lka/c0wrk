...
7. Safety signals gathered once:
     ...
     + symlink analysis (escape = hard; in-roots = not a concern; expansion suspicion removed — ADR-054)
...
- **Hard** — a fired security control: blacklist pattern match, SSRF (private/
  ...
  out of session roots (the former unresolvable/suspicious expansion escalation is removed — ADR-054). A hard reason forces
