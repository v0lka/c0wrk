...
- `core/workspace/gitconfig.go` — exec-free `.git/config` scanner (`ScanGitConfig`): dangerous-key findings (incl. the baseline-covered `signing` kind), include recording, per-repo neutralizing `-c` set
- `core/workspace/gitcommitsuppress.go` — exec-free commit-surface detection (`DetectCommitSuppression`): commit-family hooks, `core.hooksPath` marker, armed repo/global signing
...
- `backend/frontend_api_git_commit.go` — suppression-aware Commit RPC: trust check → `DetectCommitSuppression` gate → hardened/raw commit spawn with `timeouts.gitCommitTimeout` and bounded combined output
...
- **Intake warning:** when a project is switched to or an auxiliary work directory is added, `notifyGitConfigRisk` (`backend/frontend_api_gitconfig_risk.go`) scans the config and emits the global `project:git_config_risk` event when it is not provably clean (dangerous keys, include directives — deliberately not followed — malformed or unreadable config). Findings now include the baseline-covered `signing` kind (`commit.gpgsign` truthy-gated, `gpg.format`, `gpg.program`, `user.signingkey`) — reported so the intake picture is complete and the trust decision is reachable for signing-armed repositories at intake ([ADR-054](../decisions/054-commit-hooks-signing-gate.md)); hook *files* are not config and produce no intake finding — the commit gate below is what surfaces them. A clean repo emits nothing. Detection-only: neutralization in the spawn layers holds regardless of the UI. A trusted repo is rechecked against its stored snapshot on open — a drifted (or now-unreadable) config evicts the trust and re-warns with `reason` + `diff`.
...
- **Commit gate (withhold-and-decide):** the Git panel's Commit RPC (`backend/frontend_api_git_commit.go`, [ADR-054](../decisions/054-commit-hooks-signing-gate.md)) runs exec-free `DetectCommitSuppression` (`core/workspace/gitcommitsuppress.go`) on every attempt in an untrusted repository: commit-family hooks (`pre-commit`, `prepare-commit-msg`, `commit-msg`, `post-commit`; executable-bit required, `*.sample` ignored, `commondir`-resolved so linked worktrees are covered; a `core.hooksPath` finding yields a single marker without listing the redirected directory) or an *armed* (truthy) `commit.gpgsign` — repo config or the user's global config — is "armed". Armed + not forced → the commit is **withheld**: nothing spawns, no commit is created, and the result carries a non-nil `Suppressed{Hooks, SigningRepo, SigningGlobal}` with a nil error (a decision request, not a failure — the frontend renders it as a notice plus an explicit *Commit hardened (skip hooks)* button, i.e. re-commit with `force=true`; hooks/signing stay neutralized on the forced path). A detection error fails the commit closed. A trusted repository skips the gate and commits raw (hooks/signing execute — that is what trust means). Clean untrusted repositories commit as before with no new friction. Detection inherits the scanner's include posture: `include`/`includeIf` directives are recorded but never followed (ADR-033), so a commit-surface setting defined only inside an included file — armed signing above all — stays invisible and the gate does not fire (accepted; see ADR-054 "Known blind spot", and the intake warning marks every directive). The commit spawn (hardened or trusted, forced or not) gets its own budget — `timeouts.gitCommitTimeout` (default 300 s; quick probes like `rev-parse` keep 30 s) — combined stdout+stderr capture bounded at 64 KiB with a truncation marker (`Output` shows hook/signing output for trusted repos), and `cmd.WaitDelay = 1 s` so orphaned hook children cannot hold the pipes open past the timeout. `git:status_changed` is emitted only when a commit actually landed.
- **Trade-off:** in an untrusted repository, repository-defined hooks and filters — including legitimate ones like git-lfs — do not run inside c0wrk; hooks are stripped and the strip is announced in the warning notice (never silent), and since [ADR-054](../decisions/054-commit-hooks-signing-gate.md) a commit that would silently skip armed commit hooks or signing is withheld outright for an explicit decision instead of landing stripped. A trusted repository opts out — its own hooks/filters/signing run — but the opt-out is snapshot-bound and fails closed back to hardening on any config drift.
...
- **The pass announces itself before the persistent DB open.** `initProject` publishes `{state: "indexing", phase: "both"}` (the manager's own status AND an `OnProgress` emission) *before* `Service.SetProject`, because that open gob-decodes every document of every branch collection and takes **minutes** on a large index (a 6 GB / ~1M-document project in the field). Until the first progress callback the frontend otherwise has no status at all: the status bar renders nothing (`IndexingStatus` hides itself on the store's `idle` default) and the vector search panel claims "Select a project to search" while a project IS selected.
- **The No Project (CHAT) reset never blocks the project switch.** `Service.ResetForNoProject` tries the service write lock and, when a persistent DB open holds it, *queues* the reset (the open applies it as its final act, still under the lock) instead of waiting. Waiting used to hold the backend's `switchMu` for the entire open, so every CHAT/CODE toggle died on its bounded acquire (`switchLockTimeout`, 5 s) and the frontend swallows the toggle's rejected promise (`switchProjectWithState`) — an apparently dead button for minutes. The bounded-acquire failure now also emits `runtime_error` (`error_code: "project_switch_busy"`) so the failure reaches the user; readiness is dropped before the reset so a search issued in the interim never serves the outgoing project's collection.
- **The frontend seeds its index status from the getter**, not only from push events: `useVectorIndexStatus` calls `GetVectorIndexStatus` on mount, on every active-project change, and on `backend:ready`, because for an already-built index the backend emits no status event at startup — which left the store on its `idle` default (empty status-bar pill + "Select a project to search").
...
Memory program (see [ADR-049](../decisions/049-vector-index-memory-reduction.md)):

- **Content-less storage contract**: on the batch path chromem stores only `ID` + `Embedding` + the four chunk-position metadata fields (`file_path`, `start_line`, `end_line`, `language`); chunk `Content` and per-file bookkeeping (hash/size/mtime) are NOT resident — per-file facts live in the file-hash sidecar row. Search reconstructs content best-effort from the source file (`contentResolver`: bounded read ≤ `max_file_size`, `lines[start-1:end]`, per-call path→lines cache); a missing/unreadable file yields the placeholder `[content unavailable: source file missing or unreadable: <path>]`, never an error, and `must_match` filters on reconstructed text so deleted files drop out. Hydration fills only the final top-K; legacy-stored content stays authoritative where present.
- **Sidecar entry format v5** (`content_hash|size|mtime_unix_nano|chunker_fingerprint|chunk_set`, `|`-separated; 3/4-field legacy entries keep parsing): the 5th field (`idxset`) records the file's committed chunk-index set — contiguous `0..N-1` as the bare count `"N"` (empty → `"0"`), gapped sets (poison-drop survivors) as `"L:0,1,3"`. Parse is strict and capped (`1<<20` entries): malformed/oversized values downgrade the entry to legacy. An older binary's 3-or-4-field parser treats a 5-field entry as a legacy bare hash — slower but correct.
- **Enumeration without materialization**: with a known chunk set, document IDs are pure arithmetic on the deterministic `DocumentID(path, i)`; `collectDocumentIDs`, `RebuildLexical` v2, migration probing, and `ValidateCollection` stats never issue the materializing full-collection `Query(" ", N)`. Unknown set → all-or-nothing fallback to the legacy space-query. Enumeration uses a first-axis unit query vector (`QueryEmbedding`) when the embedding dimension is known, so Browse costs no ONNX inference. `RebuildLexical` v2 walks the sidecar (known fingerprint + chunk set → re-chunk → same IDs; zero ONNX) with a fallback to the collection-query path for empty/ineligible sidecars.
- **One-time content-less migration**: each collection is migrated in the background to the stripped shape exactly once, tracked by the marker `contentless_<collectionName>.done` next to the branch sidecar. The migration waits for the file-hash backfill, probes documents without the service lock, re-commits them in ≤200-doc windows with the same IDs/vectors and stripped content (chromem rewrites each per-doc gob in place), writes the marker only on full success, then frees OS memory. Cancellation (project switch, park/eviction, close, branch switch, rebuild) leaves every document individually consistent (legacy or stripped, never half) without the marker — the next open replays to completion. `IndexIncremental` waits for the migration before deleting/re-indexing.
- **Park budget in bytes**: parked state footprint is estimated as `documents × (embedding_dims × 4 B + 1 KiB)`; right after parking, the oldest parked states are evicted (sidecar flushed, then `freeOSMemory` outside the lock) while the sum exceeds `park_budget_mb` (default 1024 MiB; negative disables the byte budget; explicit 0 is rejected at load as ambiguous). The effective parked-RAM bound is `min(park_capacity, park_budget_mb)`; `park_capacity: 0` disables parking entirely regardless of the budget.
- **GC memory hygiene**: `runtime.memory_soft_limit_mb` (0/unset = AUTO = clamp(50% physical RAM, 2 GiB..8 GiB); >0 = explicit MiB; -1 = off; < -1 fails load) is resolved once at startup — precedence: off → explicit config (beats a `GOMEMLIMIT` env var, logged) → `GOMEMLIMIT` env (beats AUTO) → AUTO — and applied via `debug.SetMemoryLimit` before background indexing. `debug.FreeOSMemory` runs after every full indexing pass, after park evictions, and after the migration; incremental passes do not scavenge.
- **RSS observability**: the `GetProcessMemory` RPC feeds the StatusBar's live `RSS NNNN MB` indicator (5 s polling, exact-bytes tooltip); failures hide the indicator silently.
- Used for:
...
| `ReadImageAsDataURL(path)`          | The image viewer's image-tab counterpart to `ReadFileAsDataURL`, with the **opposite** containment contract: resolves via `resolveReadablePath` and is **not** workspace-contained, so the viewer can render an image the agent surfaced anywhere on disk (mirrors `ReadFile`). Safe because it runs only on an explicit user action (opening an image tab), never during automatic rendering. Image MIME from a deterministic `imageMimeByExt` map (png/jpg/jpeg/gif/webp/bmp/ico/svg/avif) with a `mime.TypeByExtension` fallback; 8 MiB size guard |
...
- Batch-path documents are stored content-less — ID + embedding + the four chunk-position metadata fields only; content is reconstructed best-effort from source files, and a missing/unreadable source yields a placeholder result (never an error) that `must_match` excludes (see [ADR-049](../decisions/049-vector-index-memory-reduction.md))
- A file-hash sidecar entry's chunk set is authoritative for its document IDs (deterministic `DocumentID(path, i)`): when the set is known, enumeration paths use ID arithmetic and never materialize the full collection
- The content-less migration marker is written only after every document of the collection is stripped; a cancelled migration writes no marker and replays on the next open — every document is always individually complete (fully legacy or fully stripped)
- The effective parked-RAM bound is `min(park_capacity, park_budget_mb)`; every eviction flushes the sidecar before dropping state and frees OS memory outside the lock
- The GC soft-limit resolution order is fixed — off → explicit config → `GOMEMLIMIT` env → AUTO — and every branch is logged at startup
...
| Park capacity | `vector_index.park_capacity` (default 3) | How many recently-closed projects keep their full vector-index state resident in RAM for instant return; 0 disables parking (every switch reopens from disk) |
| Park budget | `vector_index.park_budget_mb` (default 1024) | Cumulative estimated-RAM budget for ALL parked states (`documents × (embedding_dims × 4 B + 1 KiB)`); oldest parked states are evicted — sidecar flushed, OS memory freed — while the sum exceeds it; effective bound `min(park_capacity, park_budget_mb)`; negative disables the byte budget, an explicit 0 is rejected at load as ambiguous |
| Go runtime soft memory limit | `runtime.memory_soft_limit_mb` (default 0 = AUTO) | Process-level GC target applied via `debug.SetMemoryLimit` before background indexing: AUTO = clamp(50% physical RAM, 2 GiB..8 GiB); `>0` explicit MiB; `-1` off; `< -1` fails load. An explicit value beats a `GOMEMLIMIT` env var, which beats AUTO. See [ADR-049](../decisions/049-vector-index-memory-reduction.md) |
...
- [../decisions/049-vector-index-memory-reduction.md](../decisions/049-vector-index-memory-reduction.md) — memory-reduction program: GC soft limit + scavenges, sidecar v5 chunk sets, content-less storage + one-time migration, park byte budget, RSS indicator
