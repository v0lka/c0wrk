...
- The symlink analysis runs during safety-signal gathering for every non-system tool call; only escapes out of the session roots are hard reasons — the gate is a pure literal-path extractor, so the former unresolvable/suspicious expansion escalation no longer exists (ADR-060)
