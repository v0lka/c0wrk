...
The Conductor installs a `ChecklistGuardFunc` into the context (`agent.WithChecklistGuard`, built by `newChecklistGuard(planState)`) that rejects standalone (empty `step_id`) `update_checklist` calls once a plan is active **in the current Conductor run** — with one **pre-existing exception**: a standalone checklist that was already emitted *earlier in the same run* (before the plan was declared) stays updatable. The guard consults a per-run `planRunState` (its `isActive()` state — set by `conductorPublisher.Publish`, cleared on abandon; see the plan-workflow invariants) — NOT the raw blackboard plan. A plan restored from a previous (completed) task does NOT trip the guard: on a continuation, the new Conductor run starts with a fresh `planRunState` (inactive), so the continuation is free to act plan-less (standalone checklist), declare its own plan, or delegate. When it rejects, the guard returns a message instructing the agent to pass a `step_id` and to not list plan steps as checklist items. This enforces the conceptual separation between plan-level tracking (`declare_plan` / plan panel) and step-level sub-task tracking (`update_checklist`).

The exception rests on a **monotone flag**. `planRunState.standaloneChecklist` is set the first time a standalone checklist is emitted (`markStandaloneChecklist`, called from `inlineStepLifecycle.onChecklistUpdate` when `stepID == ""`) and is never cleared. Because the guard rejects an empty-`step_id` call while the plan is active unless the flag is already set, the flag is only ever set while the plan is *inactive* — before a `declare_plan`, or after an abandon deactivated an earlier one. Its live value is therefore exactly "a standalone checklist already exists earlier in this run", and no snapshot taken at declare time is needed. The practical effect: a model that laid out a plan-less checklist and then refined the work into plan steps may keep updating its original checklist instead of being retroactively rejected, while a model that declared the plan *first* can never open a new standalone checklist.
...
- The Conductor installs a `ChecklistGuardFunc` that rejects **starting** a standalone (empty `step_id`) `update_checklist` once a plan is active **in the current run**, but keeps accepting a standalone checklist emitted *earlier in the same run* — the monotone `planRunState.standaloneChecklist` flag, which can only be set while the plan is inactive (before a declare, or after an abandon) — so a plan-less checklist is never retroactively rejected when the work is later refined into plan steps. A restored plan from a previous (completed) task does NOT count as declared — the guard consults a per-run `planRunState`, not the raw blackboard plan.
...
- A user **abandon** decision in `declare_plan`'s `await_approval` mode deactivates the plan workflow for the remainder of the run: `declare_plan` calls the optional `planAbandoner` capability (`conductorPublisher.AbandonPlan()`), which (1) clears the `planRunState.declared` flag — only `declared`, since `continuable` is a run-start seed abandon can never reach (a continuable resume returns the soft "already approved" hint before the approval prompt) — and (2) settles the abandoned plan's steps by emitting one terminal `PlanStepComplete(success=false, "plan abandoned")` per step. The run therefore regains plan-less behavior: `HasDeclaredPlan()` goes false so `delegate` is re-enabled and a standalone checklist is accepted again, while `execute_plan` refuses the now-inactive plan ("not declared in this run ... abandoned at the approval prompt"). The explicit settlement is required precisely *because* the run is no longer plan-active: the finish-fallback `completeAll` is gated on `planDeclaredInRun()`, so it will no longer sweep the abandoned plan's steps, and without this they would hang "pending" in the plan panel forever. No step was started (approval precedes any execution), so no `PlanStepStart` is emitted. The abandoned plan is NOT cleared from the blackboard — it remains a historical artifact. `approve`, `present`, and **`request_changes`** all leave the workflow active: `request_changes` deliberately keeps it so the model must revise and re-declare within the same workflow (delegate stays disabled).
