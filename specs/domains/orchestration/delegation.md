...
`delegate` is for **plan-less** tasks only. Once a plan is declared via `declare_plan`, `delegate` is disabled (enforced by the PlanChecker guard) and `execute_plan` must be used to run the plan steps instead. The lock lasts until the plan workflow is released — a plan **abandoned** at the approval prompt re-enables `delegate` (see the abandon invariant in [conductor.md](conductor.md)). The two are orthogonal execution paths — `delegate` optimizes Conductor context; `execute_plan` executes a user-approved roadmap. See [conductor.md](conductor.md) for the `execute_plan` path and the `planStepEventTranslator` that adapts subagent events to plan-step lifecycle.
...
**The record.** [`core/units`](../../../core/units/units.go) defines one durable record per execution unit — `UnitRecord{ID, TaskID, Namespace, Kind, ParentID, Depth, Status, Spec, Steps}` — with `Kind` ∈ `task | plan_step | subagent | goal_verification | tool` and `Status` ∈ `pending | running | paused | completed | failed | interrupted`. `Spec` is the opaque **rebuild spec** (enough to relaunch the unit without an LLM decision) and `Steps` is the opaque **resume checkpoint** (a JSON `[]agent.Step`). `UnitStatus.Terminal()` is the single definition of "finished" (`completed`/`failed` only); `interrupted` is a unit left unfinished by a crash/app exit and carries **no** resume intent — unlike `paused` — so it is deliberately **not** terminal and the funnel relaunches it fresh (a task cancellation maps to `interrupted` too, not `failed`, so an abandoned delegation is recovered identically whether the exit was graceful or a kill). `UnitStatus.InFlight()` (`pending`/`running`) is the "abandoned by a crash" predicate, and `UnitKind.Container()` (`task`/`goal_verification`) marks a record that groups other units rather than being relaunchable work. `Begin` is **idempotent for an existing record**: a re-registration preserves the durable `CreatedAt`, `Spec`, `Steps` and status it does not restate, and every by-id lookup is namespace-scoped, so a mainline ledger can never resolve (and settle) an isolated context's same-id row. One write/read API (`Ledger`: `Begin`/`Checkpoint`/`Settle`/`List`) over one store seam (`Store`: `SaveUnit`/`SettleUnitStatusIfInFlight`/`LoadUnits`), persisted to the additive `task_units` table in the session SQLite DB (`TaskStoreAdapter.UnitStore()`, cached on the persistent blackboard). Two constructors share one task scope: `NewBlackboardLedger(bb)` for the mainline and `units.NewLedger(store, taskID, namespace)` for an isolated context (the goal verifier). A store-less run degrades to an in-memory ledger.
...
**One funnel on Resume.** `Orchestrator.Resume` ([orchestrator.go](../../../core/orchestrator.go)) enumerates the task's durable ledger (merged with legacy delegation specs for pre-ledger tasks; ledger units win by id) and settles every non-terminal **mainline** unit **uniformly**, grouped by `(scope = namespace, depth)` deepest-first:
...
Container kinds (`task`, `goal_verification`), plan-step units (owned by the plan arm) and tool units are skipped. Units recorded in an **isolated namespace** (the goal verifier's) are skipped as well: an isolated pass is re-derived by the loop that owns it, so relaunching it from the mainline funnel would write its outcome onto the **live task blackboard** (inverting the run-time isolation guarantee) and duplicate work nobody consumes. Its durable status is still surfaced through `work_units`. The terminal replay is scope-filtered per group, and a registration collision is logged rather than swallowed. Before classifying, a unit's ledger status is **reconciled with the other durable evidence**: the blackboard outcome wins when it exists (pause sentinel → `paused`, error → `failed`, success → `completed`) and a stored checkpoint paired with a non-terminal status is treated as `paused` — because `persistUnitOutcome` writes the blackboard first and the ledger second (both best-effort), a crash between the two writes must not make the funnel relaunch completed work fresh or discard a valid checkpoint. A relaunch that pauses again re-checkpoints the wave (unchanged).

**One read path.** The same ledger is surfaced to the frontend as `GetSessionRuntimeStatus.work_units`: an in-flight unit on a task that is not executing **and is not cooperatively paused** is settled `interrupted` through a column-scoped conditional store write (so it can neither drop a spec/checkpoint nor double-emit under racing pollers), container kinds are excluded exactly as the funnel excludes them, and a paused task settles nothing (its untouched tail units are what `Resume` runs). The session-load reconciliation (`reconcileWorkUnits`) aligns paused/interrupted delegate and plan-step chat blocks so an abandoned unit never stays a misleading `running` block — and the same overlay repairs the execution plan panel. See [session-lifecycle.md](../session-lifecycle.md#unified-recovery-ledger) and the `work_unit_settled` / `work_units` entries in [event-catalog.md](../../contracts/event-catalog.md).
...
- The durable **unit ledger** is the recovery source of truth. The `conductorLauncher` is its single writer; `Resume` enumerates it and settles every non-terminal **mainline** unit uniformly (paused → relaunch seeded, not-started/running/**interrupted** → relaunch fresh, terminal → replay), reconciling each ledger status against the blackboard outcome and any stored checkpoint first. An ISOLATED context's units (the goal verifier's namespace) are never relaunched by that funnel — the loop that owns them re-derives the pass — so an isolated outcome can never land on the live task blackboard. An `interrupted` unit is never marked failed without a relaunch attempt, and a cancellation is recorded as `interrupted` (not `failed`) so abandoned work is recovered on either kind of exit (see [Durable Unit Ledger and Resume Relaunch](#durable-unit-ledger-and-resume-relaunch)).
