...
### chatStore: Transcript Paging & Reading Position

In-memory, session-keyed state backing chunked transcript history and scroll restoration (behavior: [rendering.md](rendering.md) § Transcript History Paging):

| Field                              | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `historyCursor` / `historyHasMore` | Keyset cursor for the NEXT older page and whether one exists. Reset to `''`/`false` before every newest-page RPC, then recorded from the page response — or from the prepend bookkeeping when a merge kept prepended rows (below). Read synchronously from the store (not the render snapshot) by `useOlderHistoryLoader`'s guard so two fast scroll events cannot double-fetch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `historyLoading`                   | In-flight guard shared by the scroll-up loader and the plan-anchor backfill walk; cleared unconditionally per session id in `finally` so a session switch mid-fetch cannot leave it stuck and block that session's older-page loading.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `prependedHistoryIds`              | Ids of rows actually inserted by `prependHistoryMessages` (skipped duplicates are not marked). Consulted by `mergeHistoryMessages` to keep older-page rows AHEAD of a freshly loaded newest page — they are DB rows that predate `loadStartedAt` and would otherwise be dropped on a re-load, erasing the history the user scrolled up to reveal. Stale ids whose rows left the store simply never match (inert — no invalidation wired).                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `prependCursor` / `prependHasMore` | The DEEPEST prepend position per session (each prepend pages backwards, so the latest write IS the deepest). This pair survives the newest-page cursor reset above it; `ChatArea` restores `historyCursor`/`historyHasMore` from here when a merge kept prepended rows, so the next scroll-up resumes from the true continuation point instead of re-fetching rows already on screen.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
`mergeHistoryMessages` returns the number of prepended rows it kept (0 when nothing was prepended), which is what lets the caller choose between the prepend bookkeeping and the page's own cursor. `prependHistoryMessages` records ids only for genuinely inserted rows and writes the page's cursor/hasMore into both the paging fields and the prepend bookkeeping in one `set`.
