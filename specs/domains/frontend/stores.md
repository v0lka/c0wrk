...
| `paperStore`         | The paper (literature) library for the active project: the normalized library (`papers`) + an id-keyed per-paper record index (`records`, which reuses the previous record object by reference when a refresh leaves a paper unchanged), the pinned card paths (`pinned`), the reader selection (`selectedPaperId`) and the panel view mode (`library` \| `reader`), plus the RPC invoke-state (`isLoading`/`isMutating`/`error`/`lastSyncAt`). `loadLibrary` applies incrementally and resets the project-scoped state (selection + mode) only on a CROSS-PROJECT load. Backend sync lives in module-level functions (not actions): `fetchPaperLibrary` (last-started-wins against a stale cross-project payload), `applyPapersChanged` (a `papers:changed` event invalidates the loaded project's library by refetching it; an event for another project — or with nothing loaded — is ignored), `refreshPaper` (single-paper `GetPaper` upsert), `togglePaperPin` and `ensurePaperPinned` (an idempotent auto-pin — a no-op with no RPC when the paper is already pinned — used by the workspace's "Suggest hypotheses from gaps" bridge to mark the paper as prior art), and `commitFlashcardReview(paperId, cardId, grade)` (the flashcards write-back — records one self-grade through `RecordFlashcardReview`; a fail-soft no-op without a loaded library, for an unknown paper, or for a card-less id). Selectors return primitives or direct references; the pinned/filtered lists are derived with `useMemo` in `usePinnedPapers`/`useFilteredPapers`; `selectPapersSyncAt` (the `lastSyncAt` stamp) is the refresh key the `PaperWorkspace` passes to `usePaperArtifacts`, so a `papers:changed` sync rebuilds the paper's sections after a deepen. The store also owns the paper viewer tab's pseudo-path PREFIX (`PAPER_TAB_PREFIX = 'c0wrk:paper:'`, with the `paperTabPath(slug)` builder) and the `usePaperBySlug(slug)` hook the `PaperWorkspace` tab resolves its record with; `fileViewerStore.openPaper(slug)` opens that virtual tab; `usePapersResearchRoot()` exposes the effective research root the paper Compare section resolves `<research-root>/comparisons/` against. See [../../contracts/desktop-frontend.md](../../contracts/desktop-frontend.md) § Papers | No           |
