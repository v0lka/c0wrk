...
| `stepContextTokens`   | `sessionId → stepId → { used_tokens, max_tokens }` (`StepContextTokens`) — the step's own context-window totals, carried by the same step-scoped `context_fill` events; feeds the header's optional "N of M" tooltip (the fill percentage alone never enables it). `setStepContextTokens` merges partial totals — absent fields leave the previously known values instead of coercing them to 0/undefined (the event type guard does not verify these fields). |

Both maps share one lifecycle: `clearStepContextFill(sessionId)` (a slightly misleading name kept so `handlePlanGenerated` can clear both with one action) drops the session's step fills AND token totals when a new plan is generated — plan step ids are reused across plans in a session, so a step whose fill has not been re-reported yet must not briefly show the previous plan's percentage. The maps survive session switches (A→B→A) like the rest of the session-keyed state.
