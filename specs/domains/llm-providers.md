...
### ChatGPT and Kimi authentication boundary

The API-key paths remain supported: `llm.chatgpt.api_key` is an OpenAI Platform API key, and Kimi may be configured as an `llm.openai_compatible` API-key provider. These paths retain the existing masked-config behavior.

There is one narrowly scoped exception for a **personal-use ChatGPT/Codex-compatible flow**: `llm.subscriptions.chatgpt` is permitted only when `experimental.enabled` and its own `enabled` flag are both true. c0wrk supplies the verified public OAuth profile (authorization/token endpoints, client ID, scopes, callback URI, Codex service base URL) and a curated, vendor-verified model catalogue, so neither the user nor an operator configures OAuth metadata or model characteristics. The backend uses user-initiated authorization-code PKCE (`S256`), a cryptographically random state, a loopback listener bound only to `127.0.0.1`, and explicit cancellation. The OpenCode/Codex-compatible adapter calls the Responses endpoint, sends `store:false`, and may send only the configured documented account-routing header. It retries once through a forced resolver refresh after 401, while 429 quota/rate-limit responses remain classified by the sp4rk managed-provider contract.

The OpenAI Platform `GET /v1/models` endpoint is intentionally not used to discover ChatGPT/Codex subscription models: it is documented for Platform API-key authentication and reports IDs/basic ownership only, not a subscription entitlement or authoritative context, output, modality, parameter, and protocol metadata. c0wrk must not send a ChatGPT OAuth token to that distinct endpoint or enable an unknown model with fallback metadata. Catalogue additions therefore require both a verified Codex-subscription availability signal and vendor documentation sufficient to add an exact sp4rk registry record. A future documented OAuth-compatible discovery endpoint that returns account-authorized model IDs **and** complete authoritative metadata may be added behind this boundary.

Kimi is a separate managed-provider lifecycle (`llm.subscriptions.kimi`): it has independently configured authorization/token/base URLs, PKCE state, encrypted credentials, refresh, reconnect, logout, and the Kimi Code transport adapter. It is not a ChatGPT fallback and has its own 401/429 handling.

All subscription configuration is public metadata only. Access and refresh tokens are encrypted by `backend/credentials` with an AES-GCM key held in the OS secure store; they never enter YAML, Wails DTOs, logs, error strings, model-visible context, or durable agent/session memory. Status RPCs expose only provider, availability, connection state, and safe operational text.

The exception does **not** allow cookie scraping, browser-session/cache import, credential harvesting, hidden/device auth, reverse-engineered endpoints, implicit account discovery, or sending a token to an endpoint not explicitly configured by the operator. Missing experimental gate, incomplete metadata, failed state validation, corrupt credential state, missing secure-store key, or failed refresh must fail closed and require reconnect.
