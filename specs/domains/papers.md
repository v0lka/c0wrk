...
- `core/papers/skillpack.go` - the embedded `study-paper` pack and the non-destructive, versioned GLOBAL seeding run at app startup by `seedGlobalPacks` (see ADR-056).
...
- `backend/frontend_api_skills.go` - the skill cache (the c0wrk packs are seeded globally at startup; see ADR-056).
...
- The GLOBAL copy (`~/.c0wrk/.agents/skills/study-paper`) is the authoritative seeded `study-paper`: the discovery chain is project-local `<workspace>/.agents/skills` → c0wrk global `~/.c0wrk/.agents/skills` → user `~/.agents/skills` (first name wins), so the seeded copy outranks a same-named `~/.agents` skill while a project-local user copy still wins (ADR-056). `RunPaperLiterature` resolves the helper from the global seeded dir and degrades to `no_script` when it is absent.
...
| Global skill seed dir | `~/.c0wrk/.agents/skills` (`config.SkillsDir`) | Destination of the startup `seedGlobalPacks` paper-pack seed; outranks `~/.agents` in discovery (ADR-056) |
...
- [../decisions/050-papers-library.md](../decisions/050-papers-library.md) - why the study-paper skill is vendored and where the library lives (its hybrid global-seeding decision was superseded first by ADR-051, then re-decided by [ADR-056](../decisions/056-research-always-on-global-seeding.md))
- [../decisions/056-research-always-on-global-seeding.md](../decisions/056-research-always-on-global-seeding.md) - why RESEARCH is always on and every pack seeds globally at startup, with discovery reordered so the c0wrk global dir outranks `~/.agents` (supersedes [ADR-051](../decisions/051-research-pack-reconciliation.md))
