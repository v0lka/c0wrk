 ...
 *
 * The toggle state (`goalEnabled`) lives in inputModeStore but is NOT persisted
 * — goal mode is first-message-only and per-task, so persisting the toggle
 * would silently re-activate goal mode on every fresh session. It resets to
 * `false` on reload and after a successful send (see useMessageSender).
