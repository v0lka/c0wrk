...
export async function sendMessage(sessionId: string, text: string, mode: string, activeSkills: string[] = []): Promise<void> {
  try {
    const app = getApp()
    await app.SendMessage(sessionId, text, mode, activeSkills)
  } catch (err) {
